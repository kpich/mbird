"""Tests for audio interpolation implementations."""
...
def test_interpolator_returns_audio_with_correct_total_length() -> None:
    """Interpolator concatenates audio1, gap, and audio2 with correct total length."""
    interpolator = AudioDiffusionInterpolator(device="cpu")
    ...
    result = interpolator.interpolate(
        audio1, audio2, gap_duration, sample_rate=sample_rate
    )
...
def test_interpolator_handles_stereo_audio() -> None:
    """Interpolator preserves stereo channel structure."""
    interpolator = AudioDiffusionInterpolator(device="cpu")
...
def test_interpolator_preserves_input_audio_segments() -> None:
    """Interpolator keeps original audio1 at start and audio2 at end."""
    interpolator = AudioDiffusionInterpolator(device="cpu")
